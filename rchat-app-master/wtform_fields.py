...
class RegistrationForm(FlaskForm):
    ...
    position = SelectField('position', choices=["Staff", "Admin"]);
...
class VisitorRegistrationForm(FlaskForm):
    ...
    password = PasswordField('password', validators=[InputRequired(message="Password required"), Length(min=4, max=25, message="Password must be between 4 and 25 characters")])
    confirm_pswd = PasswordField('confirm_pswd', validators=[InputRequired(message="Password required"), EqualTo('password', message="Passwords must match")])
    position = SelectField('position', choices=["Visitor"]);
...
class LoginForm(FlaskForm):
    ...
    position = SelectField('position', choices=["Visitor","Staff", "Admin"]);
...
class ComplaintForm(FlaskForm):
    """ Complaint Form"""
    complaint = StringField('complaint', validators=[InputRequired(message="Please write your complaint here")]);
